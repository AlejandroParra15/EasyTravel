...
public class Algorithms {
	...
	/**
	 * Dijkstras algorithm used in order to get shortest paths from an initial
	 * vertex
	 * 
	 * @param   <V> generic value of the vertex
	...
	/**
	 * Floyd-Warshalls method to obtain the shortest path form every vertex to each
	 * other
	 * 
	...
	public static <V> String floydWarshall2(Graph<V> gg, V vec, int start, int end) {
		int[][] w = gg.getWeight();
		int n = w.length;
		int[][] D = w;
		int v = 0;
		...
		String path = printResult(D, next,start,end);
		return path;
	}

	static String printResult(int[][] dist, int[][] next, int start, int end) {
		String path = "";
		for (int i = 0; i < next.length; i++) {
			for (int j = 0; j < next.length; j++) {
				if (i != j) {
